...
// SLMT: Read (後來發現 mbbsd 的 telnet.c 根本不會用到這個 function，
// 因為它自行建立了一個 static 的 Telnet 物件)
...
ssize_t telnet_process(TelnetCtx *ctx, unsigned char *buf, ssize_t size)
{
	...
	if (size > 0) {
		...
		if (ctx->iac_state == IAC_NONE &&
				!ctx->iac_quote && memchr(buf, IAC, size) == NULL)
	    return size;
		...
		case TELOPT_NAWS:
	  {
		...
		case TELOPT_TTYPE:
			...
			if (ctx->callback->ttype && ctx->iac_buflen > 2)
				ctx->callback->ttype(ctx->ttype_arg, (char*)ctx->iac_buf+2,
						ctx->iac_buflen-2);
