...
static void init(void)
{
  ...
  /* avoid SIGPIPE */
  // SIGPIPE 是發生於程式嘗試寫入資料到損毀或被關閉的 socket 而發生
  // 預設會造成程式中止
  Signal(SIGPIPE, SIG_IGN);

  /* avoid erroneous signal from other mbbsd */
  // SIGUSR 的意義由發送的程式指定，通常用於 process 之間的同步作業
  // TODO: 了解何時會送出這些訊號
  Signal(SIGUSR1, SIG_IGN);
  Signal(SIGUSR2, SIG_IGN);
...
bool parse_argv(int argc, char *argv[], struct ProgramOption *option)
{
  ...
  // getopt() 是一種用於從 arguments 中抽取出 '-x' style option 的 function
  // "dn:p:f:l:Dt:h:e:bu:FC" 代表這個程式接受 "dnpflDthebuFC" 這些參數
  // 其中 "npfltheu" 可以額外在後面加上一個參數
  while ((ch = getopt(argc, argv, "dn:p:f:l:Dt:h:e:bu:FC")) != -1) {
    switch (ch) {
      // Enable tunnel mode (同時也有 -d 的效果)
      case 'n':
      option->tunnel_mode = true;
      if (option->flag_tunnel_path)
      {
      ...
      // optarg 是 getopt() 附帶的變數，如果 option 後面有接參數的話
      // 就會被存在這個變數內
      option->flag_tunnel_path = strdup(optarg);

      // reuse 'd' mode, no break here.

      // Use daemon mode (同時也有 -t telnet 的效果)
	    case 'd':
      given_mode = true;
      ...
      // Use non-daemon mode (同時也有 -t tty 的效果)
	    case 'D':
      given_mode = true;
      ...
      // Host IP
      // TODO: 這邊暫存在 fromhost，為什麼不存 options ?
	    case 'h':
  ...
  // 至少要指定要不要 daemon mode，這關係到用哪種 terminal mode (tty/telnet)
  if (!given_mode) {
  ...
  if (option->daemon_mode) {
    ...
    {
      // SIGCHLD 發生於 child process 終止的時候
      // TODO: 確認這裡在做甚麼
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  // 建立兩個 virtual buffers，設定一些 mmap 的參數
  init();

  // 從 argument 讀取使用者指定的 option
